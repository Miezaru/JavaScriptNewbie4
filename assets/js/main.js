...
let wordsList = (str, subStr) => {
  ...
  let arr = new Set();
  for (let item of set) {
    if (item.indexOf(subStr) > -1) arr.add(item);
  }
  return arr;
};
let myLongStr =
...
let myDate = new Date(2019, 06, 16, 0, 15, 32);
...
//   let result;
//   if (!isISO) {
//     return (result = isSeconds
//       ? date.toLocaleString()
//       : date.toLocaleString().replace(reg, ''));
//   } else if (date && isSeconds && isISO) {
//     return `${date.getFullYear()}.${month}.${day}, ${hours}:${minutes}:${seconds}`;
//   } else if (date && isISO) {
//     return `${date.getFullYear()}.${month}.${day}, ${hours}:${minutes}`;
//   } else if (date && isSeconds) {
...
let getLocalDate = (date, isSeconds, isISO) => {
  ...
  if (!isISO) {
    result = isSeconds
      ? date.toLocaleString()
      : date.toLocaleString().replace(/:\d{2}$/gi, '');
  } else {
    result = isSeconds
...
console.log(getDateAgo('2020-01-29', 1)); // 28.01.2019
